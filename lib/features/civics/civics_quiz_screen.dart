...
class CivicsQuizScreen extends ConsumerStatefulWidget {
  final String sectionId;

  const CivicsQuizScreen({super.key, required this.sectionId});

  @override
  ConsumerState<CivicsQuizScreen> createState() => _CivicsQuizScreenState();
}

class _CivicsQuizScreenState extends ConsumerState<CivicsQuizScreen> {
  static const Color _themeColor = Colors.purple;
  static const Color _themeDark = Color(0xFF6A1B9A);
  ...
  Future<void> _loadQuizzes(String sectionId) async {
    try {
      final jsonStr =
          await rootBundle.loadString('assets/data/quizzes_civics.json');
