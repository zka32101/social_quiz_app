...
class WorldQuizScreen extends ConsumerStatefulWidget {
  const WorldQuizScreen({super.key});

  @override
  ConsumerState<WorldQuizScreen> createState() => _State();
}

class _State extends ConsumerState<WorldQuizScreen> {
  static const Color _primary = Color(0xFF0277BD);
  ...
  Widget build(BuildContext context) {
    final async = ref.watch(_worldQProvider);
    return Scaffold(
      appBar: AppBar(
        title: const Text('🌏 世界地理クイズ'),
              ...
              Expanded(
                child: ClipRRect(
                  ...
                  child: TweenAnimationBuilder<double>(
                    tween: Tween(
                      begin: _idx / qs.length,
                      end: (_idx + 1) / qs.length,
                    ),
