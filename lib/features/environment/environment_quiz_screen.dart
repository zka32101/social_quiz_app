...
final _envQProvider =
  ...
  final raw = await rootBundle.loadString('assets/data/quizzes_environment.json');
...
Color _sectionColor(String id) {
  const colors = {
    'pollution': Color(0xFF795548),
    'climate': Color(0xFFE53935),
    'sdgs': Color(0xFF00897B),
    'energy': Color(0xFFF9A825),
    'recycle': Color(0xFF43A047),
    'forest': Color(0xFF2E7D32),
  };
  return colors[id] ?? const Color(0xFF00897B);
}

class EnvironmentQuizScreen extends ConsumerStatefulWidget {
  final String sectionId;

  const EnvironmentQuizScreen({super.key, required this.sectionId});

  @override
  ConsumerState<EnvironmentQuizScreen> createState() => _State();
}

class _State extends ConsumerState<EnvironmentQuizScreen> {
  int _idx = 0;
  int? _sel;
  ...
  Color get _primary => _sectionColor(widget.sectionId);
  ...
  Widget build(BuildContext context) {
    final async = ref.watch(_envQProvider(widget.sectionId));
    return Scaffold(
      appBar: AppBar(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              mainAxisAlignment: MainAxisAlignment.center,
              children: List.generate(3, (i) => _EnvAnimatedStar(
                filled: i < stars,
...
class _EnvQuizBody extends StatelessWidget {
  final _Q q;
  final int? sel;
  final bool answered;
  final bool isLast;
...
class _EnvAnimatedStar extends StatefulWidget {
  final bool filled;
  final Duration delay;
  final Color color;

  const _EnvAnimatedStar({required this.filled, required this.delay, required this.color});

  @override
  State<_EnvAnimatedStar> createState() => _EnvAnimatedStarState();
}

class _EnvAnimatedStarState extends State<_EnvAnimatedStar> with SingleTickerProviderStateMixin {
